...
namespace jcom
{
	...
	void jalr::ParseToken()
	{
		...
		while (mFile.Available())
		{
			...
			{
				if (mStack.top() == jdesc::ExpressionList)
				{
					if (mFile.PeekNext().content != context.breaker)
					{
						mStack.push(jdesc::Expression);
						IncTree();
						mStack.push(jdesc::Term);
						tokenState = jTagGate::Open;
					}
				}
			}
			...
			switch (tokenState)
			{
				...
				case(jTagGate::Close):
				{
					...
					if (mStack.top() == jdesc::Subroutine || mStack.top() == jdesc::ExpressionList)
					{
